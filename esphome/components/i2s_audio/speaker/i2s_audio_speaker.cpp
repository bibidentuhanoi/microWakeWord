...
// Single core write function handling both data and optional finish
        size_t I2SAudioSpeaker::write(const uint8_t *data, size_t length, bool finish)
        {
            ...
            // Handle end-of-stream signal (length==0 && finish=true)
            if (finish && length == 0)
            {
                ESP_LOGD(TAG, "End-of-stream signal received; triggering finish logic");
                this->interrupted_ = false;

                // Clear stale semaphore
                xSemaphoreTake(this->completion_semaphore_, 0);

                // Block until completion or interrupt
                bool completed = (xSemaphoreTake(this->completion_semaphore_, portMAX_DELAY) == pdTRUE);
                return completed ? 1 : 0;  // 1 for success, 0 for failure
            }
                ...
                (this->state_ == state_t::RUNNING || this->buffer_->available() > 0))
            {
                ...
                ESP_LOGI(TAG, "Auto-interrupting playback: Switching from task '%s' to '%s'", old_name, new_name);

                // Release mutex temporarily to avoid deadlock if interrupt needs it
                xSemaphoreGive(this->buffer_mutex_);
                this->interrupt();
                // Re-acquire mutex
                if (xSemaphoreTake(this->buffer_mutex_, pdMS_TO_TICKS(100)) != pdTRUE)
                {
                    ESP_LOGW(TAG, "Failed to re-acquire buffer mutex after auto-interrupt");
                    return 0;
                }
            }
            ...
            if (written > 0)
            {
                ...
                this->last_task_handle_ = current_task;
            }
